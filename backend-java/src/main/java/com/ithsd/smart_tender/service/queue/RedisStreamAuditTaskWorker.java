...
public class RedisStreamAuditTaskWorker {
    ...
    private void handleFailure(String messageId, String taskId, int retry, RuntimeException ex) {
        ...
        if (nextRetry > Math.max(0, queueProperties.getMaxRetry())) {
            redisTemplate.opsForStream().add(MapRecord.create(queueProperties.getDlqStreamKey(), Map.of(
                    "taskId", taskId,
        ...
        redisTemplate.opsForStream().add(MapRecord.create(queueProperties.getStreamKey(), Map.of(
                "taskId", taskId,
