    ...
    fn definition(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "function": {
                ...
                "description": "【使用场景】校验投标保证金或履约保证金的合规性——\
                    ① 投标保证金金额是否≤合同金额2%且不超过50万(货物服务)/80万(工程)；\
                    ② 履约保证金是否≤合同金额10%；\
                    ③ 保证金形式是否为法定接受形式(现金/保函/保证保险)；\
                    ④ 退还时限是否符合法定要求(投标保证金≤5工作日)。\
                    【不使用场景】不负责审核保证金的具体退还流程和退还条件细节；\
                    不负责审核招标文件中保证金条款的文字表述是否规范。\
                    【法条依据】《政府采购法实施条例》第33条、第48条。\
                    【注意】金额单位为万元（人民币）。deposit_type 为 'bid' 时校验投标保证金规则，\
                    为 'performance' 时校验履约保证金规则。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "deposit_amount": {
